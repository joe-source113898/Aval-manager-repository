...
import { ClientesManager } from "@/components/admin/clientes/clientes-manager";
import { cn } from "@/lib/utils";
import { getRoleFromSession, isAdminRole, isAdvisorRole } from "@/lib/auth";
import { getServerSupabase } from "@/lib/supabase/server";

export default async function AsesoresClientesPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-10">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border/60 bg-muted/10 px-4 py-3">
        ...
        <p className="text-sm text-muted-foreground">Estás gestionando: Clientes</p>
      </div>
      <ClientesManager />
    </div>
  );
